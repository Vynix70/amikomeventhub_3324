...
<head>
    ...
    <title>E-Ticket #{{ $transaction->order_id }} - AmikomEventHub</title>
...
<body class="bg-indigo-600 text-white min-h-screen flex items-center justify-center p-6">
    ...
    <div class="max-w-md w-full">
        <div class="text-center mb-8 class-print-hide">
        ...
        <div class="bg-white text-slate-900 rounded-[2.5rem] overflow-hidden shadow-2xl relative">
            ...
            <div class="p-8 bg-indigo-50 border-b-4 border-dashed border-indigo-100 text-center relative">
                ...
                <h2 class="text-2xl font-black leading-tight text-indigo-950">
                    {{ $transaction->event->title ?? 'Nama Event Tidak Tersedia' }}
                </h2>
            ...
            <div class="p-8 space-y-8">
                <div class="grid grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <p class="font-bold text-lg text-slate-800">
                            {{ $transaction->event ? \Carbon\Carbon::parse($transaction->event->date)->translatedFormat('d M Y') : '-' }}
                        </p>
                    </div>
                    <div>
                ...
                <div class="bg-slate-100 p-6 rounded-3xl flex flex-col items-center">
                    ...
                    <p class="mt-4 font-mono font-bold text-slate-800 tracking-wider">TKT-{{ str_pad($transaction->id, 8, '0', STR_PAD_LEFT) }}</p>
                </div>
            </div>

            <div class="px-8 pb-8 space-y-3 block-print-hide">
                <button onclick="window.print()"
                    class="w-full py-4 bg-indigo-600 text-white rounded-2xl font-bold shadow-lg hover:bg-indigo-700 transition active:scale-[0.98]">
                    Cetak / Simpan PDF
                </button>
                <a href="{{ route('home') }}"
                    class="block text-center text-sm text-slate-500 font-bold hover:text-indigo-600 transition pt-1">
                    Kembali ke Beranda
                </a>
            </div>
        </div>
    ...
    <style>
        @media print {
            .block-print-hide, .class-print-hide {
