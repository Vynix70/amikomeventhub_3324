...
<main class="max-w-3xl mx-auto px-6 py-20">
    <div class="mb-12">
        ...
        <p class="text-slate-500 mt-2">Lengkapi data Anda untuk menyelesaikan pemesanan.</p>
    </div>
    ...
    @php
        $unitPrice = $tier ? $tier->price : 0;
        $totalTicketPrice = $unitPrice * $quantity;
        $serviceFee = $totalTicketPrice == 0 ? 0 : 5000;
        $grandTotal = $totalTicketPrice + $serviceFee;
    @endphp
    ...
    <div class="grid grid-cols-1 gap-8">
        ...
        <div class="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm">
            ...
            <div class="flex gap-6 items-start">
                ...
                <div>
                    <span class="inline-block px-2.5 py-0.5 bg-indigo-50 text-indigo-600 font-bold text-xs rounded-full uppercase mb-1">
                        Tier: {{ $tier->name ?? 'Reguler' }}
                    </span>
                    ...
                    <p class="text-indigo-600 font-bold mt-2">
                        {{ $quantity }} x {{ $unitPrice == 0 ? 'Gratis' : 'Rp ' . number_format($unitPrice, 0, ',', '.') }}
                    </p>
                </div>
            </div>
            ...
            <div class="mt-8 pt-6 border-t space-y-3">
                <div class="flex justify-between text-slate-500">
                    ...
                    <span>{{ $totalTicketPrice == 0 ? 'Gratis' : 'Rp ' . number_format($totalTicketPrice, 0, ',', '.') }}</span>
                </div>
                
                <div class="flex justify-between text-slate-500">
                    <span>Biaya Layanan</span>
                    @if($totalTicketPrice == 0)
                        <span class="text-emerald-600 font-bold uppercase text-xs">Bebas Biaya</span>
                    @else
        ...
        {{-- Input Voucher (Hanya jika event berbayar) --}}
        @if($totalTicketPrice > 0)
        <div class="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm">
            ...
            <div class="flex gap-3">
                <input type="text" id="voucher_input" 
                    value="{{ $voucherCode ?? '' }}"
                    class="flex-1 px-5 py-4 border-2 border-slate-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none transition font-medium uppercase" 
        ...
        <div class="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm">
            ...
            <form action="{{ route('checkout.store', $event->id) }}" method="POST" class="space-y-6">
                @csrf
                <input type="hidden" name="ticket_tier_id" value="{{ $tier->id ?? '' }}">
                <input type="hidden" name="quantity" value="{{ $quantity }}">
                <input type="hidden" name="voucher_code" id="hidden_voucher_code" value="{{ $voucherCode ?? '' }}">

                <!-- Nama Pelanggan (Otomatis dari Google Auth) -->
                <div>
                    ...
                    <input type="text" name="customer_name" id="customer_name" 
                        class="w-full px-5 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:outline-none transition font-medium text-slate-600 cursor-not-allowed"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <!-- Email Pelanggan (Otomatis dari Google Auth) -->
                    <div>
                ...
                <button type="submit" id="btn_submit_checkout"
                    ...
                    {{ $grandTotal == 0 ? 'Daftar Event Gratis Sekarang' : 'Lanjut Pembayaran' }}
                </button>
                <p class="text-center text-xs text-slate-400 mt-2">Dengan menekan tombol di atas, Anda menyetujui Syarat & Ketentuan kami.</p>
            </form>
        </div>
    </div>
</main>
...
function applyVoucher() {
    ...
    if (!voucherCode) {
        ...
        msgElement.innerText = "Silakan ketik kode voucher terlebih dahulu!";
    ...
    .then(data => {
        msgElement.classList.remove('hidden');
        if (data.success) {
            msgElement.className = "text-sm font-semibold mt-3 text-emerald-600";
            ...
            // Kunci field input & tombol voucher
            voucherInput.setAttribute('disabled', 'true');
            voucherInput.classList.add('bg-slate-50', 'text-slate-400', 'cursor-not-allowed');
...
// Jika kode voucher dikirim dari query parameter halaman detail, jalankan penerapannya secara otomatis
if (document.getElementById('voucher_input').value.trim() !== '') {
    applyVoucher();
}
</script>
@endif
